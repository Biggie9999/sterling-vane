...
export default function DocumentsPage() {
  return (
    <div className="space-y-8">
      <div>
        <p className="font-mono text-[10px] uppercase tracking-widest text-[#C9A84C] mb-2">Investor Vault</p>
        ...
        return (
          <div key={cat}>
            ...
            <div className="space-y-2">
              {catDocs.map((doc) => (
                <div key={doc.name} className="flex items-center justify-between gap-4 bg-[#111] border border-[#222] rounded-xl p-5 hover:border-[#C9A84C]/30 transition-colors group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-[#C9A84C]/10 rounded-xl flex items-center justify-center shrink-0">
                      <doc.icon className="w-5 h-5 text-[#C9A84C]" />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider text-[#C9A84C] border border-[#C9A84C]/30 px-3 py-2 rounded-lg hover:bg-[#C9A84C]/10 transition-colors">
